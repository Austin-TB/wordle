...
function initializeGame() {
    ...
    rows.forEach(row => {
        row.querySelectorAll('.wordle-input').forEach(input => {
            input.value = '';
...
// Function to activate a specific row for input
function activateRow(rowIndex) {
    const inputs = rows[rowIndex].querySelectorAll('.wordle-input');
    inputs[0].focus()
    ...
    inputs.forEach((input, index) => {
        input.addEventListener('input', () => {
            ...
            entry = `${entry}${input.value}`;
            if (input.value.length === 1 && index < inputs.length - 1) {
                inputs[index + 1].focus();
                resetGameButton.style.backgroundColor = 'pink';
            }
        });
        ...
        input.addEventListener('keydown', (event) => {
            ...
            // Move to the next row on pressing enter
            if (event.key === "Enter" && index === inputs.length - 1 && input.value !== '') {
                if (currentRowIndex < rows.length) {
                    checkEntry(Array.from(entry), inputs);
                    if (win) {
                        ...
                        rows[currentRowIndex].querySelectorAll('.wordle-input').forEach(input => {
                            input.disabled = true;
                        });
                        currentRowIndex++; // Increment to the next row index
                        activateRow(currentRowIndex); // Activate the next row
                    }
                }
            }
        });
...
function checkEntry(entry, inputs) {
    let correct = 0;
    for (let i = 0; i < 5; i++) {
        if (entry[i] === answer[i]) {
            inputs[i].style.backgroundColor = 'lightgreen';
            correct++;
        } else if (answer.includes(entry[i])) {
            inputs[i].style.backgroundColor = 'gold';
        } else {
